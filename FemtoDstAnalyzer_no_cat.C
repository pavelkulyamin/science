...
//_________________
void FemtoDstAnalyzer_no_cat (const Char_t *inFile = "st_physics_12150008_raw_4030001.femtoDst.root", const Char_t *outFileName = "outFile.root") {
  ...
  for(Long64_t iEvent=0; iEvent<events2read; iEvent++) {
   ...
   //  	       << "/" << events2read << "]" << std::endl;
    ...
    /* if( TMath::Abs( ( event -> primaryVertex().Z() ) - ( event -> vpdVz() ) - 0.0339) > 2.89*1.5 || 
        event -> cent9() >= 1) {
         continue;
	 }*/
    ...
    for(Int_t iTrk=0; iTrk<nTracks; iTrk++) {
      ...
      /* if( femtoTrack->gMom().Mag() < 0.1 || femtoTrack->gDCA(pVtx).Mag() > 3. ) {
       ...
       if( TMath::Abs( femtoTrack -> eta() ) > 1 || femtoTrack -> nHits() < 15 || 
          femtoTrack -> p() < 0.1) {
         continue;
	 }*/
